...
impl<E: PairingEngine> TableInput<E> {
    pub fn store(&self, path: &str) {
        // 1. Polynomial
        let mut o_bytes = vec![];
        let mut f = File::create(path).expect((format!("Unable to create file at {path}")).as_str());
...
mod tests {
    ...
    fn do_multiple_lookups<E: PairingEngine>() {
        ...
        const MIN_LOG_N: usize = 10;
        const MAX_LOG_N: usize = 14;
        const EPS: usize = 1;
        const MIN_LOG_M: usize = 3;
        const MAX_LOG_M: usize = 8;
        ...
        for n in MIN_LOG_N..=MAX_LOG_N {
            ...
            println!("Time to generate poly and openings for size {:?} = [{:?}]", actual_degree, now.elapsed());
            ...
            for logm in MIN_LOG_M..=std::cmp::min(MAX_LOG_M, n / 2 - 1) {
                ...
                println!("Time to generate lookup proof if size {:?} for = [{:?}]", m, now.elapsed());
                let now = Instant::now();
                ...
                println!("Time to verify lookup proof of size {:?} for  = [{:?}]", m, now.elapsed());
                assert!(res);
